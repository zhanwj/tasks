...
class ResNet_convX_body(nn.Module):
    def __init__(self, block_counts):
        ...
        if len(block_counts) == 4:
            ...
            self.res5, dim_in = add_stage_deform(dim_in, 2048, dim_bottleneck * 8, block_counts[3],
...
def add_stage_deform(inplanes, outplanes, innerplanes, nblocks, dilation=1, stride_init=2):
    ...
    for i in range(nblocks):
        ...
        res_blocks.append(add_residual_block_deform(
...
def add_residual_block_deform(inplanes, outplanes, innerplanes, dilation, stride):
    """Return a residual block module, including residual connection, """
    if stride != 1 or inplanes != outplanes:
        shortcut_func = globals()[cfg.RESNETS.SHORTCUT_FUNC]
        downsample = shortcut_func(inplanes, outplanes, stride)
    ...
    res_block = bottleneck_transformation_deform(
        inplanes, outplanes, innerplanes, stride,
        dilation=dilation, group=cfg.RESNETS.NUM_GROUPS,
        downsample=downsample)
...
class bottleneck_transformation_deform(nn.Module):
