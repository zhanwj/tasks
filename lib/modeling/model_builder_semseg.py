...
class SegmentationModuleBase(nn.Module):
    def __init__(self):
        ...
        self.big_class = [0, 2, 8]

    def pixel_acc(self, pred, label):
        _, preds = torch.max(pred, dim=1)
        valid = (label!=255).float()
        #key_label = valid * (label!=0).float() * (label!=2).float() * (label!=8).float() 
        acc_sum = (preds == label).float()
        #key_sum = acc_sum * key_label
        pixel_sum = torch.sum(valid)
        #key_pixel_sum = torch.sum(key_label)
        acc = torch.sum(acc_sum) / (pixel_sum + 1e-10)
        #key_acc = torch.sum(key_sum) / (key_pixel_sum + 1e-10)
        return acc


class Generalized_SEMSEG_BN(SegmentationModuleBase):
    def __init__(self):
        super(SegmentationModuleBase, self).__init__()
    ...
    def forward(self, feed_dict, *, segSize=None):
        ...
        if self.training: # training
            ...
            if cfg.SEM.DECODER_TYPE.endswith('deepsup') or cfg.SEM.DECODER_TYPE.startswith('spn'): # use deep supervision technique
                (pred_semseg, pred_deepsup) = self.decoder(self.encoder(feed_dict['data'], return_feature_maps=True))
                if not isinstance(pred_deepsup, list):
                    pred_deepsup = [pred_deepsup]
            else:
                pred_semseg = self.decoder(self.encoder(feed_dict['data'], return_feature_maps=True))
